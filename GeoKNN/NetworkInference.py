...
class NetworkInference:
    ...
    def __init__(self):
        ...
        self.Sinit = None
        self.Sfinal = None
        self.Available_Inference_Methods = ['Standard_oCSE', 'Alternative_oCSE', 'Lasso', 'Graphical_Lasso', 
    ...
    def Entropy_GKNN(self,X):
        ...
        for i in range(N):
            ...
            KforMean[i] = len(Wh2[0])
            #This is the final terms combined from the paper
            Term3[i] = np.sum(np.log(SS*Epsilons[i]))
        
        Term2 = np.mean(np.log(KforMean))
        Term3 = np.mean(Term3)
